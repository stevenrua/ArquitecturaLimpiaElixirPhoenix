defmodule MatricularCursoCa.Utils.CustomTelemetry do
  alias MatricularCursoCa.Utils.DataTypeUtils
  import Telemetry.Metrics

  @service_name Application.compile_env!(:matricular_curso_ca, :custom_metrics_prefix_name)
  ...
  def custom_telemetry_events() do
    :telemetry.attach("matricular_curso_ca-plug-stop", [:matricular_curso_ca, :plug, :stop], &handle_custom_event/4, nil)
    :telemetry.attach("matricular_curso_ca-redis-stop", [:redix, :pipeline, :stop], &handle_custom_event/4, nil)
    :telemetry.attach("matricular_curso_ca-vm-memory", [:vm, :memory], &handle_custom_event/4, nil)
  ...
  def handle_custom_event([:matricular_curso_ca, :plug, :stop], measures, metadata, _) do
    :telemetry.execute(
